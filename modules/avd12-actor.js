...
export class Avd12Actor extends Actor {
  ...
  hasFocusEquipped() {
    ...
    if (allEquippedItems.length > 0) return true;
    return false;
  }
  ...
  getTraits(type) {
    return [...foundry.utils.duplicate(this.items.filter((item) => item.type === "trait" && item.system.traittype === type) || []), ...foundry.utils.duplicate(this.tmpTraits.filter((trait) => trait.system.traittype === type) || [])];
  }
  ...
  async addObjectToContainer(itemId, containerId) {
    ...
    } else if (object && object.system.containerid) {
  ...
  async channelSpell(channeledSpell, userData) {
    ...
    if (channeledSpell.unlinkedToken) currentFocusPoints = channeledSpell.unlinkedToken.system.focus.currentfocuspoints;
    ...
    if (rollData.spellCost > currentFocusPoints) {
      ...
      if (spell.system.spelltype != "utility") {
        if (channeledSpell.unlinkedToken) channeledSpell.unlinkedToken.update({ "system.focus.currentfocuspoints": channeledSpell.unlinkedToken.system.focus.currentfocuspoints - rollData.spellCost });
        else this.update({ "system.focus.currentfocuspoints": this.system.focus.currentfocuspoints - rollData.spellCost });
  ...
  async getSpellById(id) {
    if (this.type == "character") {
      ...
      let comp = foundry.utils.duplicate(this.items.filter((item) => item.type == "spell") || []);
      comp.forEach((spell) => {
        if (spell._id == id) foundSpell = spell;
      });
      return foundSpell;
    } else if (this.type == "npc") {
      return this.items.get(id);
  ...
  getMove(id) {
    for (let i = 0; i < this.tmpActions.length; i++) {
      if (id == this.tmpActions[i].system.avd12_id) return this.tmpActions[i];
    }
    for (let i = 0; i < this.tmpReactions.length; i++) {
      if (id == this.tmpReactions[i].system.avd12_id) return this.tmpReactions[i];
    }
    for (let i = 0; i < this.tmpFreeActions.length; i++) {
      if (id == this.tmpFreeActions[i].system.avd12_id) return this.tmpFreeActions[i];
    }
    for (let i = 0; i < this.tmpBallads.length; i++) {
      if (id == this.tmpBallads[i].system.avd12_id) return this.tmpBallads[i];
    }
  }
  ...
  getPowerPercentage() {
    ...
    if (percentage <= 0) return { primary: "0%", secondary: "0%" };
